...
function ADC(five, callback, opt) {
  if (!opt) {
    opt = { gas: 5, flame: 5, alarm: 20, gasLevel: 750, flameLevel: 700 };
  }
  var onGasAlarm = false;
  var onFlameAlarm = false;
  var alarmTime = opt.alarm ? opt.alarm : 60;
  // AD Converter
  var ADConverter = new five.Board.Virtual(new five.Expander("PCF8591"));
  ...
  water.on("change", function () {
    if (val != Math.ceil(this.value / 10)) {
      val = Math.ceil(this.value / 10);
      callback("ADC", 2, val);
    }
    // console.log("water value", this.value);
  });
